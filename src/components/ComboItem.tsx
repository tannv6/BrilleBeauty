...
export default function ComboItem({ info }: any) {
  return (
    <div className="font-Arial flex flex-col">
      <div className="h-[216px] border border-[#dbdbdb] rounded-2xl relative group">
        <Image className="object-cover rounded-2xl" src={`${CDN_URL}${info.ComboImage}`} alt="" fill />
        ...
        <div className="absolute hidden top-0 w-full h-full bg-black-rgba z-10 rounded-2xl items-center justify-center gap-x-5 group-hover:flex">
      ...
      <div className="p-5 pb-10">
        <Link href={`/combo/view/${info.ComboID}`} className="font-bold text-[18px]">
          {info.ComboName}
        </Link>
        <p className="pt-2">
          <span className="line-through text-[#bbbbbb]">A${info.InitPrice||"0"}</span>
          <span className="pl-[10px] text-[20px] font-bold">A${info.SellPrice||"0"}</span>
