...
export const getServerSideProps = async (context: any) => {
  ...
  const response = await axios.get(
    ...
    {
      params: { depth: 3, session: JSON.stringify(session) },
    }
  );
...
export default function Face({
  ...
  const selectedOptionType = optionTypes.find(
    (optionType: any) => optionType.PotID === product.PotID
  );

  const [dropdownState, setDropdownState] = useState({
    activeItem:
      product.ProductID ||
      (optionTypes2.length > 0 ? optionTypes2[0]?.ProductID : null),
  });
  ...
  const totalOptionPrice = Object.keys(selectedOptions).reduce(
    (total, optionId) => {
      if (selectedOptions[parseInt(optionId)]) {
        const option = optionTypes2.find(
          ...
          <div className="py-5" key={option.PoID}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-row">
                <button
                  ...
                  onClick={() => {
                    updateNumProduct(
                      option.PoID,
                      Math.max((NumProduct[option.PoID] || 1) - 1, 0)
                    );
                  }}
                ></button>
                <input
                  type="number"
                  value={NumProduct[option.PoID] || 1}
                  onChange={(e) => {
                    updateNumProduct(option.PoID, Number(e.target.value));
                  }}
              ...
              <div className="flex flex-row items-center gap-2">
                ...
                <button
                  className="rounded w-[33px] h-[33px] bg-[url('/product_detail/product_del_btn.png')]"
                  onClick={() => handleDeleteOption(option.PoID)}
                ></button>
              </div>
            </div>
          </div>
        ...
        <div className="inner-container mt-[70px] mb-[60px]">
                ...
                <div className="flex items-center">
                  ...
                  <Dropdown
                    ...
                    options={optionTypes2
                      .filter(
                        (option: any) => option.ProductID === product.ProductID
                      )
                      .map((option: any) => ({
                        id: option.PoID,
                        name: option.PoName,
                      ...
                      <div className="flex basis-[20%] items-start justify-end gap-[10px]">
                        {userInfo &&
                          ...
                          e.UserID === userInfo.CustomerID && (
                            <>
                              <Link
                                href={`/write_review/${e.ReviewID}?ProductID=${product.ProductID}`}
          ...
          <div className="relative">
                        ...
                        <div className="flex flex-col">
                          ...
                          >
                            <Image
                              ...
                              alt="1"
                            ></Image>
                            <p className="font-bold pt-4 truncate overflow-hidden whitespace-pre-wrap line-clamp-2 min-h-[52px]">
                              {e.ProductName}
                            </p>
                            <p className="pt-1">
                              <span className="line-through text-[15px] text-[#bbbbbb]">
                                A${e.InitPrice}
                              </span>
            ...
            {numSlides > 5 && (
              <>
                <button
                  className="absolute top-1/4 left-[-56px] w-[36px] h-[37px] bg-[url('/product_rlt_arrow_prev.png')]"
