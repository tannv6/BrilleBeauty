...
export async function getServerSideProps({ params, query }: any) {
  ...
  const response2 = await axios.get("http://localhost:3000/api/combo_category/current", {
...
export default function ComboSearch({ combo, cateName, page, total, totalPage }: any) {
        ...
        <div id="main">
          <div className="w-full h-[340px] bg-[url('/sub_face/main_visual.png')]">
            <div className="flex justify-center flex-col gap-5 h-full pl-[415px]">
              <p className="text-black text-[32px] font-[700]">{cateName.data[0]?.CategoryName}</p>
          ...
          <div className="inner-container mt-[50px]">
            <div className="w-full flex justify-between mb-[60px]">
              <p className="w-full text-[30px] text-[#ef426f] text-center font-bold">{cateName.data[0]?.CategoryName}</p>
            </div>
