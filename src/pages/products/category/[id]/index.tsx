...
export async function getServerSideProps(context: any) {
  ...
  const response = await axios.get(
    ...
    {
      params: { page, sort, brand, pageSize: 12, cate_id: params.id, depth: 1 , session: JSON.stringify(session)},
    }
  );
  const response2 = await axios.get(
    "http://localhost:3000/api/category/current",
    {
      params: { cate_id: params.id, depth: 1 },
    }
  );
          ...
          <div className="w-full flex flex-wrap gap-3 mb-[60px] text-[18px] text-[#656565]">
            ...
            {subCate.data?.map((e: any, i: any) => {
              return (
                <Link
                  key={i}
                  href={`/products/category/${id}/${e.CategoryID}`}
                  scroll={false}
                >
